# TODOs (do not include in commits)
- [x] add blurbs on "assessing reference URL/URI stability"
- [x] add blurb on "using permanent identifiers - DOIs, and the like
- [x] add blurb on choosing the best title for a reference when referencing a web page
- [ ] add note on making reference consistent wrt the version of the reference; for example pre-print vs. conference paper vs. journal article
- [ ] add more examples and reasoning for code repo references
- [ ] Should NIST be added as a publisher of SPs? -> YES
- [ ] add templates and examples where needed
- [ ] add note that this page will be updated and may change over time (or something like that)
...
Notes:
maybe move the assessing URL stability blurb to a "best practices" section earlier on this page; that way things like that don't get buried. Then the tail of the article is just templates and examples.

# Scrap (do not include in commits)
...
Removed this because it's in the weeds; come back later
**Specifics for the Cryptology ePrint Archive:**
(from ["How the Cryptology ePrint Archive Operates"](https://eprint.iacr.org/operations.html))

> Posting a paper to the Cryptology ePrint Archive does not prevent future or
> concurrent submission to any journal or conference with proceedings. In this
> respect, the papers in the Cryptology ePrint Archive have the status of
> technical reports.

TODO Possibly contradicting guidance for the Cryptology ePrint Archive
(from ["How to Cite a Paper in the Cryptology ePrint Archive"](https://eprint.iacr.org/citation.html))

> Authors should cite the original work if it is published elsewhere. If at all
> possible, use the DOI for the paper to identify it uniquely.

Possible outcome: when in doubt follow the BibTeX provided at the page; add DOI
if there is a specific version you want to point to? (But that feels
inconsistent...maybe just simpler to say "follow whats on the page")
...
CMOS reference for this that may be helpful:

[CMOS 1.122](https://www.chicagomanualofstyle.org/book/ed18/part1/ch01/psec122.html):

> Like “in press” or “forthcoming” articles accepted by journals but not yet
> edited, these are not to be confused with the final, edited electronic articles
> published ahead of a print issue (see 1.123).
# Notes for the team to review (do not include in commits)

- review blurb on assessing reference URL/URI stability
- review language on titles for webpages
- review language on "latest version" vs "this version" notes
- order of contents - what read betters?
